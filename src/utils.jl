"""
    check_cfl(vmax, dt, dx, dy)

Check the Courant-Friedrichs-Lewy stability condition.
For 2D: C = vmax * dt * sqrt(1/dx² + 1/dy²) < 1
"""
function check_cfl(vmax::Real, dt::Real, dx::Real, dy::Real)
    courant = vmax * dt * sqrt(1.0 / dx^2 + 1.0 / dy^2)
    if courant > 1.0
...
"""
    suggest_dt(vmax, dx, dy; courant_target=0.5)

Suggest a stable time step for given grid spacing and max velocity.
"""
function suggest_dt(vmax::Real, dx::Real, dy::Real; courant_target=0.5)
    return courant_target / (vmax * sqrt(1.0 / dx^2 + 1.0 / dy^2))
end
